...
# Создадим словарь со значениями букв для английского языка
eng_values = {
...
# Создадим словарь со значениями букв для русского языка
rus_values = {
...
# Получим от пользователя слово и определим, на каком языке оно написано
word = input('Введите слово: ').upper()

if 'A' <= word[0] <= 'Z': # Если первая буква - английская
    values = eng_values
else: # Иначе - русская
    values = rus_values
...
X = int(input("Введите число, количество вхождений которого необходимо вычислить: "))
count = 0
for i in A:
    if i == X:
        count += 1
